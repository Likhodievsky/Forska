...
namespace Forska
{
    public partial class Form2 : Form
    {
        ...
        double[] SubIntgrFunTopA = new double[1000];
        double[] SubIntgrFunBottA = new double[1000];
        double[] SIntgrFun1 = new double[1000];
        double[] SIntgrFun = new double [50];
        int[,] m;
        int NodesNb = 0;
        int treeCount;
        int Spl = 10 * 10;
        ...
        public void draw_tree(int x, int y, double d1, double d2)
        {
            ...
            g.DrawEllipse(new Pen(Brushes.Brown, 1), new Rectangle(new Point(x - Convert.ToInt32(d2) / 2 + Convert.ToInt32(d2) / 2 - Convert.ToInt32(d1) / 2, y - Convert.ToInt32(d2) / 2 + Convert.ToInt32(d2) / 2 - Convert.ToInt32(d1) / 2), new Size(Convert.ToInt32(d1), Convert.ToInt32(d1))));
        }
        public void rost(int colTree)
        {
            ...
            for (int i = 0; i < colTree; ++i)
            {
                ...
                for (int k = 0; k <= i; k++)
                {
                    if (x >= (m[k, 0] - mass[k].L * 20*5) && x <= (m[k, 0] + mass[k].L * 20*5) && y >= (m[k, 1] - mass[k].L * 20*5) && y <= (m[k, 1] + mass[k].L * 20*5))
                    {
                        flag = false;
                        break;
                    }
                ...
                if (flag)
                {
                    ...
                    mass[i].x = x;
                    mass[i].y = y;
                    draw_tree(x, y, mass[i].D, mass[i].L);
                }
                else i--;
            }
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            for (int q = 0; q <= trackBar2.Value; q++)
            {
                ...
                textBox2.AppendText(MaxCanopyH.ToString() + "\r\n");
                textBox2.AppendText(MinCanopyH.ToString() + "\r\n");
                ...
                for (int i = 0; i < treeCount; i++)
                {
                    ...
                    int k = 0;
                    while (z[k] < mass[i].CrownTop + 0.01 && k < z.Length)
                    {
                        k++;
                        mass[i].TopNode = k;
                    }
                ...
                for (int i = 0; i < treeCount; i++)
                {
                    ...
                    double Aux2 = Aux1 * 2;
                    mass[i].dH = Math.Abs(Intgr / (mass[i].D * (mass[i].D + mass[i].H * Aux2)));
                    mass[i].dD = Math.Abs(Intgr / (mass[i].D * (mass[i].H + mass[i].H + mass[i].D / Aux1)));
                    mass[i].dL = Math.Abs(mass[i].Cbig * Intgr / (mass[i].H + mass[i].D / Aux2) - mass[i].psi * mass[i].L);
            ...
            for (int i = 0; i < treeCount; i++)
            {
                draw_tree(mass[i].x, mass[i].y, mass[i].D, mass[i].L);
            }
